...
# wake the backend as soon as the app loads, before the user clicks anything 
@st.cache_resource
def wake_backend():
    try:
        requests.get(ROOT_URL, timeout=60)
    except Exception:
        pass  # fine if this fails silently, retry logic is the real safety net

wake_backend()

# retry helper for the actual prediction call ---
def call_predict_api(payload, max_retries=5, delay=8):
    last_error = None
...
with col2:
    ...
    if submit:
        ...
        try:
            with st.spinner("Waking up the model server — first request after idle time can take up to a minute..."):
            ...
            st.error(f"Could not connect to backend server after multiple attempts. Error: {e}")
    else:
        st.info("Adjust values on the left pane and click 'Analyze Risk Profile' to generate an assessment.")